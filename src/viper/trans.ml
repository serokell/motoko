...
let rec unit (u : M.comp_unit) : (IntSet.t * prog) Diag.result =
  Diag.(
    reset_stamps();
...
and unit' (u : M.comp_unit) : (IntSet.t * prog) =
...
and tr_typ' ctxt typ =
  ...
  | _, T.Tup  ts ->
    ctxt.tuple_arities := IntSet.add (List.length ts) !(ctxt.tuple_arities);
